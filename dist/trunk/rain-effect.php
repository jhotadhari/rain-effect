...
class Rain_Rain_effect {
	...
	public function on_activate() {
		if ( $this->check_dependencies() ){
			...
			do_action('rain_on_activate_before_flush'); 			
			flush_rewrite_rules();
			$this->maybe_update();
			do_action('rain_plugin_activated');
		} else {
			add_action( 'admin_init', array( $this, 'deactivate' ) );
			wp_die(
	...
	public function on_deactivate() {
		flush_rewrite_rules();
		do_action('rain_plugin_deactivated');
	}
	
	public static function on_uninstall() {
		do_action('rain_plugin_uninstalled');	
	}
	...
	protected function init_options() {
		update_option( 'rain_version', self::VERSION );
		add_option( 'rain_db_version', self::DB_VERSION );
	}
	...
	protected function maybe_update() {
		if ( get_option( 'rain_db_version' ) < self::DB_VERSION ) {
			// require_once( self::plugin_dir_path() . 'inc/dep/class-rain-update.php' );
			// new Rain_Update();
			// class Rain_Update()  is missing ??? !!!
		}
	}
